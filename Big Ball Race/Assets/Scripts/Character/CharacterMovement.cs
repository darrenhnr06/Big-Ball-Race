...
{   
    ...
    private void Update()
    {
        ...
        if (!joystickClick)
        {
            if ((Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Began && Balls.Count >= 1 && powerBar3D.GetComponent<BarMeter>().GetBarTouch()) || (Input.GetMouseButtonDown(0) && Balls.Count >= 1 && powerBar3D.GetComponent<BarMeter>().GetBarTouch()))
            {
    ...
    IEnumerator TouchTimer()
    {
        ...
        while(powerBar3D.GetComponent<BarMeter>().ReturnBarMeterRunning())
        {
            yield return new WaitForSeconds(0.1f);
        }

        powerBar3D.GetComponent<BarMeter>().SetBarTouch(false);

        if (powerBar3D.GetComponent<BarMeter>().ReturnPerfectHit() == true)
        {
            throwSpeed = 16;
        }
        else
    ...
    IEnumerator ThrowBall()
    {
        ...
        powerBar3D.gameObject.SetActive(false);
        ...
        holdingBall.transform.localScale = new Vector3(1.535f, 1.535f, 1.535f);
    ...
    IEnumerator StartCountTouch()
    {
        ...
        if (Balls.Count > 0)
        {
            ...
            yield return new WaitForSeconds(1f);
            powerBar3D.gameObject.SetActive(true);
            powerBar3D.GetComponent<BarMeter>().SetBarRunning(true);
            powerBar3D.GetComponent<BarMeter>().SetBarTouch(true);
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.CompareTag("Ball Throw Area") && Balls.Count >= 1)
        {
            ...
            transform.DOLocalMove(ballThrowPos.position, 0.2f);
